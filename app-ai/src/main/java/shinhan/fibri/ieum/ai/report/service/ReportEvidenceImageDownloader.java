...
public class ReportEvidenceImageDownloader {
	...
	public VerifiedReportEvidenceImage download(ReportReviewImage image) {
		...
		try (body) {
			...
			if (response.headers().firstValueAsLong("Content-Length").orElse(0L) > maxBytes) {
			...
			byte[] bytes = readBoundedWithinDeadline(body, deadlineNanos);
			if (!isWebp(bytes)) {
	...
	private byte[] readBoundedWithinDeadline(InputStream body, long deadlineNanos) {
		...
		Future<byte[]> future = executor.submit(() -> readBounded(body));
		try {
